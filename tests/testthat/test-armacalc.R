...
## im_arma2p2 <- InformationMatrixARMA(phi22, -theta22)
## saveRDS(im_arma2p2, "tests/testthat/im_arma2p2.rds", version = 2)
im_arma2p2 <- readRDS("im_arma2p2.rds")
expect_equal(.FisherInfo(-phi22, theta22), im_arma2p2, check.attributes = FALSE)

phi21 <- c(0.8, 0.1) 
theta21 <- 0.6
## arma(2,1)
## im_arma2p1 <- InformationMatrixARMA(phi21, -theta21)
## saveRDS(im_arma2p1, "tests/testthat/im_arma2p1.rds", version = 2)
im_arma2p1 <- readRDS("im_arma2p1.rds")
expect_equal(.FisherInfo(-phi21, theta21), im_arma2p1, check.attributes = FALSE)

## arma(1,2)
## im_arma1p2 <- InformationMatrixARMA(-theta21, phi21)
## saveRDS(im_arma1p2, "tests/testthat/im_arma1p2.rds", version = 2)
im_arma1p2 <- readRDS("im_arma1p2.rds")
expect_equal(.FisherInfo(theta21, -phi21), im_arma1p2, check.attributes = FALSE)

## ar(2)
## im_ar2 <- InformationMatrixARMA(phi21)
## saveRDS(im_ar2, "tests/testthat/im_ar2.rds", version = 2)
im_ar2 <- readRDS("im_ar2.rds")
expect_equal(.FisherInfo(-phi21), im_ar2, check.attributes = FALSE)

## ma(2)
## im_ma2 <- InformationMatrixARMA(theta = -phi21)
## saveRDS(im_ma2, "tests/testthat/im_ma2.rds", version = 2)
im_ma2 <- readRDS("im_ma2.rds")
expect_equal(.FisherInfo(theta = phi21), im_ma2, check.attributes = FALSE)

phi11 <- 0.8 
theta11 <- 0.5
## arma(1,1)
## im_arma1p1 <- InformationMatrixARMA(phi11, -theta11)
## saveRDS(im_arma1p1, "tests/testthat/im_arma1p1.rds", version = 2)
im_arma1p1 <- readRDS("im_arma1p1.rds")
expect_equal(.FisherInfo(-phi11, theta11), im_arma1p1, check.attributes = FALSE)
## ar(1)
## im_ar1 <- InformationMatrixARMA(phi11)
## saveRDS(im_ar1, "tests/testthat/im_ar1.rds", version = 2)
im_ar1 <- readRDS("im_ar1.rds")
expect_equal(.FisherInfo(-phi11), im_ar1, check.attributes = FALSE)
## ma(1)
## im_ma1 <- InformationMatrixARMA(theta = -theta11)
## saveRDS(im_ma1, "tests/testthat/im_ma1.rds", version = 2)
im_ma1 <- readRDS("im_ma1.rds")
expect_equal(.FisherInfo(theta = theta11), im_ma1, check.attributes = FALSE)
...
## if(requireNamespace("FitARMA")){
## this uses BJ convention, negate coef's (the above use SP convention): 
## d <- InformationMatrixARMA(-c(-0.8, 0.1), -0.5)
## saveRDS(d, "tests/testthat/d_ex_InformationMatrixARMA.rds", version = 2)
d <- readRDS("d_ex_InformationMatrixARMA.rds")
