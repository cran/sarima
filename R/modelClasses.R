...
setMethod("modelCoef", c("VirtualFilterModel", "missing"),
          function(object){
              filterCoef(object)
          }
          )

setMethod("modelCoef", c("VirtualFilterModel", "character"),
          function(object, convention){
...
setMethod("modelCoef", c("VirtualFilterModel", "BJ"),
          function(object, convention){
...
setMethod("modelCoef", c("VirtualFilterModel", "SP"),
          function(object, convention){
              if(class(object) == "ArmaModel")
                 filt <- modelCoef(object)
              else{
...
setMethod("modelCoef", c("VirtualFilterModel", "BD"),
          function(object, convention){
              if(class(object) == "ArmaModel")
                 modelCoef(object)
              else{
...
setMethod("modelCoef", c("SarimaModel", "ArModel"),
          function(object, convention){
              wrk <- filterPolyCoef(object, lag_0 = FALSE)
              if(length(wrk$fullmapoly) > 0)
                   stop("Model not Ar-like (has non-trivial moving average part)")
...
setMethod("modelCoef", c("SarimaModel", "MaModel"),
          function(object, convention){
              wrk <- filterPolyCoef(object, lag_0 = FALSE)
              if(length(wrk$fullarpoly) > 0)
                  stop("Model not MA-like (has non-trivial autoregressive part)")
